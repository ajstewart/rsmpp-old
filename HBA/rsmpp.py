...
#Written by Adam Stewart, Last Update November 2013

#---Version 1.61---
#Patched peeling to work with a user defined skymodel.

#---Version 1.6---
#Added the ability to use a more up to date build of the lofar software. (This makes life easier with AWimager)
#Added fix for mosaics when using rsm-mainline build of the software.
#Added precalibrated mode
#Added new options for phase only mode.
...
vers="1.62"	#Current version number

#Check and import functions file
if not os.path.isfile("rsmpp_funcs.py"):
	subprocess.call(["cp", "/home/as24v07/scripts/rsmpp/HBA/rsmpp_funcs.py", "."])	
import rsmpp_funcs as rsmf

#Check environment
curr_env=os.environ
if curr_env["LOFARROOT"] not in rsmf.correct_lofarroot:
	print "\nCorrect Environment, as of 12/12/2013, has not been loaded!\n\
Please run the following commands and then retry:\n\n\
For rsm-mainline:\n\
. /opt/soft/reset-paths.sh\n\
. /opt/rsm-mainline/init-lofar.sh\n\n\
For September 2013 LOFAR:\n\
. /opt/soft/reset-paths.sh\n\
. /opt/share/lofar/init-september-build.sh\n\n\
For December 2013 LOFAR:\n\
. /opt/soft/reset-paths.sh\n\
. /opt/share/lofar/init-lofar.sh\n\n\
It may be the case that the build has been updated since last rsmpp version."
	sys.exit()

chosen_environ=rsmf.correct_lofarroot[curr_env["LOFARROOT"]]
print "Running on {0} version of software".format(chosen_environ)

#Check for parset file
config_file="rsmpp.parset"
if not os.path.isfile(config_file):
	subprocess.call(["cp", "~as24v07/scripts/rsmpp/HBA/rsmpp.parset", "."])
	print "The parset file 'rsmpp.parset' could not be found so a default version has been copied to the current directory.\n\
...
group.add_option("--precalibrated", action="store_true", dest="precalib", default=config.getboolean("GENERAL", "precalibrated"),help="Select this option if the data has been precalibrated by ASTRON [default: %default]")
group.add_option("--precalibratedloc", action="store", type="choice", choices=['DATA', 'CORRECTED_DATA',], dest="precalibloc", default=config.get("GENERAL", "precalibratedloc"), help="Define whether the calibrated data is located in the DATA or CORRECTED_DATA column [default: %default]")
...
group.add_option("-j", "--target_oddeven", action="store", type="string", dest="target_oddeven", default=config.get("GENERAL", "target_oddeven"),help="Specify whether the targets are the odd numbered observations or the even [default: %default]")
...
group.add_option("-P", "--PHASEONLY", action="store_true", dest="PHASEONLY", default=config.getboolean("GENERAL", "PHASEONLY"),help="Choose just to perform only a phase only calibration on an already EXISTING rsmpp output [default: %default]")
group.add_option("--phaseonly_name", action="store", type="string", dest="phase_name", default=config.get("GENERAL", "phaseonly_name"),help="Specifcy the name of the output directory of the phase only mode [default: %default]")
group.add_option("--phaseonly_col", action="store", type="choice", dest="phase_col", choices=['DATA', 'CORRECTED_DATA',], default=config.get("GENERAL", "phaseonly_col"),help="Choose which column to pull the data from ('DATA' or 'CORRECTED_DATA') [default: %default]")
group.add_option("--phaseonly_bands", action="store", type="string", dest="phase_pattern", default=config.get("GENERAL", "phaseonly_bands"),help="Specifcy the glob pattern to select the bands to perform the phase-cal on (eg. '??' selects L*/L*BAND??_*.dppp) [default: %default]")
...
image_meth=options.imagemeth #image using casa or AWimager
...
data_dir=options.data	#directory where data to process is located
...
resume=options.resume	#resume mode on off
...
#if resume (from a crash) we can import the completed steps bit
if resume:
...
if phaseO:
	...
	try:
		...
		standalone_phase_multi=partial(rsmf.standalone_phase, phaseparset=phaseparset, flag=flag, toflag=toflag, autoflag=autoflag, create_sky=create_sky, skymodel=skymodel, phaseoutput=phase_name, phasecolumn=phase_col)
...
Script now exiting...".format(i, data_dir))
		...
		for i,j in izip(target_obs, calib_obs):
			...
			for b in beams:
				...
				rsmf.check_targets(i, b, targets, targets_corrupt, rsm_bands, rsm_band_numbers, rsm_bands_lens, missing_calibrators, data_dir, diff, g, subsinbands)
		...
		for i,j in izip(sorted(targets.keys()), sorted(calibs)):
			...
			NDPPP_Initial_Multi=partial(rsmf.NDPPP_Initial, wk_dir=working_dir, ndppp_base=ndppp_base, prec=precal, precloc=precalloc)
			if __name__ == '__main__':
				if not precal:
					if "NDPPP_{0}".format(j) not in completed_steps:
						worker_pool.map(NDPPP_Initial_Multi, calibs[j])
						completed_steps.append("NDPPP_{0}".format(j))
					calibs[j] = sorted(glob.glob(os.path.join(j,"*.tmp")))
					if "NDPPP_{0}_CHECK".format(j) not in completed_steps:
						...
						checkresults=worker_pool.map(rsmf.check_dataset, calibs[j])
						...
						corrupt_report.close()
						completed_steps.append("NDPPP_{0}_CHECK".format(j))
			...
			for b in beams:
				...
				if nchans==0:
					...
					temp.close()
				if "NDPPP_{0}_{1}_CHECK".format(i, beam) not in completed_steps:
					if not os.path.isfile("post_ndppp_corrupt_report.txt"):
						corrupt_report=open("post_ndppp_corrupt_report.txt", 'w')
						corrupt_report.close()
					corrupt_report=open("post_ndppp_corrupt_report.txt", "a")
					...
					corrupt_report.close()
					completed_steps.append("NDPPP_{0}_{1}_CHECK".format(i, beam))
			...
			if not precal:
				...
				if __name__ == '__main__':
					if "BBS1_{0}".format(j) not in completed_steps:
						worker_pool.map(calibrate_msss1_multi, calibs[j])
						completed_steps.append("BBS1_{0}".format(j))
			else:
		...
		# 	# snapshot_concat_multi=partial(rsmf.snapshot_concat, beam=be)	#Currently cannot combine all bands in a snapshot (different number of subands)
			final_concat_multi=partial(rsmf.final_concat, beam=be, target_obs=target_obs, correct=correct)
			if __name__ == '__main__':
				if "CONCAT_BEAM{0}".format(be) not in completed_steps:
					worker_pool.map(final_concat_multi, rsm_band_numbers)
					completed_steps.append("CONCAT_BEAM{0}".format(be))
		completed_steps.append("FINAL_CONCAT")
		...
		if imaging_set:
			...
			if mosaic:
				create_mosaic_multi=partial(rsmf.create_mosaic, band_nums=rsm_band_numbers, chosen_environ=chosen_environ, pad=userpad)
