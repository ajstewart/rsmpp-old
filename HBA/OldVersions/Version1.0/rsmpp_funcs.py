import os, subprocess,time, multiprocessing, glob
...
def convert_newawimager(environ):
	environ['LOFARROOT']="/opt/share/lofar-archive/2012-12-18-14-29/LOFAR_r23371_74ff14b"
	environ['PATH']="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/opt/soft/pathdirs/bin:/opt/share/lofar-archive/2012-12-18-14-29/pathdirs/bin"
	environ['LD_LIBRARY_PATH']="/opt/soft/pathdirs/lib:/opt/share/lofar-archive/2012-12-18-14-29/pathdirs/lib"
	environ['PYTHONPATH']="/opt/soft/pathdirs/python-packages:/opt/share/lofar-archive/2012-12-18-14-29/pathdirs/python-packages"
	return environ

def check_targets(i, beam, targets, last_subbands, rsm_bands, rsm_band_numbers, missing_calibrators, data_dir, diff, missingfile):
	...
	last_subbands[i+"_"+beamselect]=targets_last
...
def calibrate_msss1(Calib, beams, diff, calparset, calmodel, correctparset, dummy, quiet):
	"""
...
# 	print "Obtaining Gain Tables for {0}...".format(calib_name)
# 	parmdbmfile="parmdbm_transfer_{0}".format(calib_name)
# 	gains=open(parmdbmfile, 'w')
# 	gains.write("open tablename='{0}/instrument'\n\
# export Gain* tablename='{0}.table'\n\
# exit".format(Calib))
# 	gains.close()
# 	subprocess.call("parmdbm < {0} > {1}/logs/parmdbm_gains_{2}.log 2>&1".format(parmdbmfile, curr_obs, Calib[-flen:]), shell=True)
# 	os.remove(parmdbmfile)
	for beam in beams:
		if beam==0:
...
def final_concat(b, beam, quiet):
	...
	subprocess.call("~as24v07/scripts/concat.py SAP00{0}_BAND{1}_FINAL.MS L*/L*SAP00{0}_BAND{1}.MS.dppp > logs/concat_SAP00{0}_band{1}.log 2>&1".format(beam,'%02d' % b), shell=True)
...
def AW_Steps(g, aw_sets, aw_env, quiet):
	"""
...
image={0}.img\n".format(g))
	...
	subprocess.call("awimager {0} > logs/awimager_{1}_log.txt 2>&1".format(aw_parset_name, logname), env=aw_env, shell=True)
	subprocess.call("image2fits in={0}.img.restored.corr out={0}.img.fits > logs/image2fits.log 2>&1".format(g), shell=True)
