...
if curr_env["LOFARROOT"] != rsmf.correct_lofarroot:
	print "\nCorrect Environment, as of 18/02/2013, has not been loaded!\n\
...
if toprocess!="to_process.py":
	toprocess_list=toprocess.split().sort()
#get the beams
...
if image==True:
	...
	if image_meth=="AW" or image_meth=="BOTH":
		...
		for s in aw_sets:
			...
			if "threshold=" in s:
				to_remove.append(s)
			if "select=\"sumsqr" in s:
				maxbaseline=np.sqrt(float(s.split("<")[-1].replace("\"", "").rstrip()))
		for j in to_remove:
			aw_sets.remove(j)
		print "Maximum baseline to image: {0}m".format(maxbaseline)
		AW_Steps_multi=partial(rsmf.AW_Steps, aw_sets=aw_sets, mask_size=mask_size, maxb=maxbaseline, aw_env=awimager_environ, niter=niters, quiet=qu)
		if __name__ == '__main__':
