...
def standalone_phase(target, phaseparset, flag, toflag, autoflag, create_sky, skymodel, phaseoutput):
	"""
	Simply shifts the CORRECTED_DATA to a new measurement set DATA column.
	"""
	tsplit=target.split("/")
...
def final_concat(band, beam, target_obs, correct):
	...
	for ms in toconcat:
		temp=pt.table("{0}/SPECTRAL_WINDOW".format(ms))
...
UVmax={3}\n".format(g, initialiter, 6.*bandsthreshs_dict[curr_band],UVmax))
		...
		try:
			thresh=3.*(getimgstd("{0}.img.fits".format(g)))
		except:
...
correct_lofarroot='/opt/share/lofar-archive/2013-04-16-11-29/LOFAR_r23543_10c8b37'