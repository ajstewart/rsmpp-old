import os, subprocess,time, multiprocessing, glob, datetime
...
def AW_Steps(g, aw_sets, mask_size, aw_env, quiet):
	"""
...
mask={1}\n".format(g, mask))
	...
	subprocess.call("awimager {0} > {1}/logs/awimager_{2}_log.txt 2>&1".format(aw_parset_name, obsid, logname), env=aw_env, shell=True)
	subprocess.call("addImagingInfo {0}.img.restored.corr parsets/SAP{1}.sky 0 6000 {0} > {2}/logs/addImagingInfo_{3}_log.txt 2>&1".format(g, '%03d' % beam, obsid, logname), shell=True)
