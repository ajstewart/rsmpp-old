...
if curr_env["LOFARROOT"] != rsmf.correct_lofarroot:
	print "Correct Environment has not been loaded!\n\
...
'L81111,L81112,L81113,L81114,...' [default: %default]")
...
group.add_option("-l", "--beams", action="store", type="string", dest="beams", default="0,1,2,3,4,5,6", help="Use this option to select which beams to process in the format of a list of beams with no spaces \
...
#Check presence of to_process.py
...
if calmodel=="AUTO":
	...
	if not os.path.isfile(calmodel):
		print "Cannot locate {0}, please check your calmodel file is present\n\
If you would like to automatically fetch the calibrator skymodel do not use the -s option.".format(skymodel)
		print "Script now exiting..."
		sys.exit()
	else:
		create_cal=False	
...
for i,j in izip(sorted(targets.keys()), sorted(calibs)):
	...
	calibrate_msss1_multi=partial(rsmf.calibrate_msss1, beams=beams, diff=diff, calparset=calparset, calmodel=calmodel, correctparset=correctparset, dummy=dummy, oddeven=target_oddeven, firstid=firstid_oe, quiet=qu)
	if __name__ == '__main__':
		worker_pool.map(calibrate_msss1_multi, calibs[j])

print "Done!"
#Combine the bands
print "Creating Bands for all sets..."
...
for be in beams:
	...
	final_concat_multi=partial(rsmf.final_concat, beam=be, target_obs=target_obs, rsm_bands_lens=rsm_bands_lens, quiet=qu)
	if __name__ == '__main__':
...
if image==True:
	...
	if image_meth=="AW" or image_meth=="BOTH":
		...
		for j in to_remove:
			aw_sets.remove(j)
		AW_Steps_multi=partial(rsmf.AW_Steps, aw_sets=aw_sets, mask_size=mask_size, aw_env=awimager_environ, niter=niters, quiet=qu)
		if __name__ == '__main__':
	...
	average_band_images_multi=partial(rsmf.average_band_images, beams=beams, quiet=qu)
	if __name__=='__main__':
		worker_pool.map(average_band_images_multi, target_obs)
	if mosaic:
		create_mosaic_multi=partial(rsmf.create_mosaic, band_nums=rsm_band_numbers, quiet=qu)
		for i in target_obs:
...
if destroy:
	print "Destroy Mode Selected, now deleting mostly everything..."
	for c in calib_obs:
...
if mail==True:
	if user=="mp":
		em.send_email(emacc,user_address,"slofarpp Job Completed","Gosia!,\n\nYour job {1} has been completed - finished at {2} UTC with a runtime of {3}\n\nTeraz spit eely, przepraszam, znachy isc i look at it. Chce dzisiaj wyniki!\n\nCzerwony?".format(user,newdirname, date_time_end, tdelta))
	else:
