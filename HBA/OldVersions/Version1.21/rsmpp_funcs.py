...
def check_targets(i, beam, targets, rsm_bands, rsm_band_numbers, rsm_bands_lens, missing_calibrators, data_dir, diff, missingfile, subsinbands):
	...
	for t in targets[i][beamselect]:
		...
		if miss==False:
			...
			rsm_bands_lens[i+"_"+beamselect+"_{0}".format(target_bandno)]=len(rsm_bands[i+"_"+beamselect+"_0"])
			# if 0 <= SB_cal < 12:
...
def AW_Steps(g, aw_sets, mask_size, aw_env, niter, quiet):
	"""
...
threshold={3}Jy\n".format(g, mask, finish_iters, thresh))
	...
	subprocess.call("image2fits in={0}.img.restored.corr out={0}.img.fits > {1}/logs/image2fits.log 2>&1".format(g, obsid), shell=True)
	subprocess.call("addImagingColumns.py {0}.img.restored.corr parsets/SAP{1}.sky 0 6000 {0} > {2}/logs/addImagingInfo_{3}_log.txt 2>&1".format(g, '%03d' % beam, obsid, logname), shell=True)
...
# correct_environ={'MIRPROG': '/opt/soft/builds/miriad-atnf/prog', 'MIRARCH': 'linux64', 'MIRBIN': '/opt/soft/builds/miriad-atnf/linux64/bin', 'LIBPATH': ':/usr/local/ups/lib', 'SSH_CLIENT': '152.78.192.60 54000 22', 'LOFARROOT': '/opt/share/lofar-archive/2013-02-07-13-37/LOFAR_r23543_10c8b37', 'LOGNAME': 'as24v07', 'USER': 'as24v07', 'HOME': '/home/as24v07', 'PATH': '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/opt/share/soft/pathdirs/bin:/opt/share/lofar-archive/2013-02-07-13-37/pathdirs/bin', 'MIR': '/opt/soft/builds/miriad-atnf', 'LD_LIBRARY_PATH': '/opt/share/soft/pathdirs/lib:/opt/share/lofar-archive/2013-02-07-13-37/pathdirs/lib', 'LANG': 'en_GB.UTF-8', 'TERM': 'xterm-256color', 'SHELL': '/bin/bash', 'XDG_SESSION_COOKIE': 'eb08b04266050531467273c700000b64-1360235115.335132-719512290', 'LANGUAGE': 'en_GB:en', 'SHLVL': '1', 'DISPLAY': 'localhost:10.0', 'MIRCAT': '/opt/soft/builds/miriad-atnf/cat', 'SHLIB_PATH': ':/usr/local/ups/lib', 'MIRPDOC': '/opt/soft/builds/miriad-atnf/doc', 'PYTHONPATH': '/opt/share/soft/pathdirs/python-packages:/opt/share/lofar-archive/2013-02-07-13-37/pathdirs/python-packages', 'MIRLIB': '/opt/soft/builds/miriad-atnf/linux64/lib', 'MIRARCHD': '/opt/soft/builds/miriad-atnf/linux64', 'MIRINC': '/opt/soft/builds/miriad-atnf/inc', 'MIRDEF': '.', 'LESSOPEN': '| /usr/bin/lesspipe %s', '_': '/usr/bin/ipython', 'MIRMAN': '/opt/soft/builds/miriad-atnf/linux64/man', 'SSH_CONNECTION': '152.78.192.60 54000 152.78.192.143 22', 'LESSCLOSE': '/usr/bin/lesspipe %s %s', 'SSH_TTY': '/dev/pts/0', 'OLDPWD': '/opt/share/lofar-archive', 'PWD': '/home/as24v07/RAIDE/RSM_Processing', 'MAIL': '/var/mail/as24v07', 'LS_COLORS': '', 'MIRSUBS': '/opt/soft/builds/miriad-atnf/subs'}
correct_lofarroot='/opt/share/lofar-archive/2013-02-11-13-55/LOFAR_r23543_10c8b37'