...
#Written by Adam Stewart, Last Update April 2013

#---Version 1.4---
#Arguments now passed through a parset.

#---Version 1.31---
#Now checks for corrupted datasets
#Cope with obsids of 7 length
...
group.add_option("-q", "--loglevel", action="store", type="string", dest="loglevel", default=config.get("GENERAL", "loglevel"),help="Use this option to set the print out log level ['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG'] [default: %default]")
...
group.add_option("-p", "--peeling", action="store", type="int", dest="peeling", default=config.getint("PEELING", "peelingno"),help="Use this option to enable peeling, specifying how many sources to peel [default: %default]")
...
group.add_option("-i", "--image", action="store",type="string", dest="image", default=config.get("IMAGING", "imgmeth"),help="Use this option to image the results with AWimager - 'AW' or Casa - 'CASA', enter settings in a textfile \
...
group.add_option("-l", "--maxbaseline", action="store", type="int", dest="maxbaseline", default=config.getint("IMAGING", "maxbaseline"),help="Enter the maximum baseline to image out to (IN m) [default: %default]")
...
log.info("All measurement sets will be checked for any corruption, hence what follows will be a lot of table open print statements")
time.sleep(2)
...
if topeel:
	...
	peeling_steps_multi=partial(rsmf.peeling_steps, shortpeel=shortpeel, peelsources=peelsources_todo)
...
if image==True:
	...
	if image_meth=="AW" or image_meth=="BOTH":
		...
		AW_Steps_multi=partial(rsmf.AW_Steps, aw_sets=aw_sets, maxb=maxb, aw_env=awimager_environ, niter=niters, initialiter=initialiters, nomask=no_mask, mos=mosaic)
		if __name__ == '__main__':
