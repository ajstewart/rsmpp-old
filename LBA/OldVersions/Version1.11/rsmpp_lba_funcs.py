...
import os, subprocess,time, multiprocessing, glob, datetime, pyfits, logging
import numpy as np
...
def NDPPP_Initial(SB, wk_dir, ndppp_base):
	"""
...
def peeling_steps(SB, shortpeel, peelsources):
	"""
...
def AW_Steps(g, aw_sets, maxb, aw_env, niter, initialiter, nomask):
	"""
	...
	beamc="SAP00{0}".format(beam)
	finish_iters=niter-initialiter
...
niter={1}\n\
threshold={2}Jy\n\
UVmax={3}".format(g, finish_iters, thresh, UVmax))
	if not nomask:
		local_parset.write("mask={0}\n".format(mask))
	for i in aw_sets:
		local_parset.write(i)
	...
	subprocess.call("image2fits in={0}.img.restored.corr out={0}.img.fits > {1}/logs/image2fits.log 2>&1".format(g, obsid), shell=True)
	subprocess.call("addImagingInfo.py {0}.img.restored.corr parsets/SAP{1}.sky 0 {3} {0} > {2}/logs/addImagingInfo_{3}_log.txt 2>&1".format(g, '%03d' % beam, obsid, logname, maxb), shell=True)
...
def create_mosaic(snap, band_nums):
	for b in band_nums:
...
correct_lofarroot='/opt/share/lofar-archive/2013-03-06-11-47/LOFAR_r23543_10c8b37'